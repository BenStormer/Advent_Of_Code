 ...
 *    - For the first NUM_CONNECTIONS entries in the BTree HashMap, look up
 *      coordinates in the group HashMap.
 *       - If one of the coordinates is already in a group, add the other one
 *         to the same group
 *       - If neither coordinate is in a group, create a new group with them
 * 4. Repeat until all coordinates are sorted in groups
 * 5. Count the number of coordinates in each group and store in new BTree
 *    Set (since it is sorted)
 * 6. Multiply the # of coordinates in the 3 largest groups
 */
